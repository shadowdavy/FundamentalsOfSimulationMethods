...
void calc_forces(particle * p, int ntot, double boxsize, double rcut)
{
    ...
    for (i = 0; i < ntot; i++) {
        for (j = i + 1; j < ntot; j++) {        // TODO: Neighbour list modification has to be added here
            for (k = 0, r2 = 0; k < 3; k++) {
    ...
    for (i = 0; i < ntot; i++) {
        for (j = i + 1; j < ntot; j++) {
            ...
            */
            ...
            if (sqrt(r2) < 0.5 * boxsize) {
                n = (int)(sqrt(r2) / ((0.5 * boxsize) / n_rdf_grid));
                rdf_grid[n] += 2;
            }
...
int main(int argc, char **argv) {
    ...
    int start_rdf = 4000;                           // steps to start RDF calculation
