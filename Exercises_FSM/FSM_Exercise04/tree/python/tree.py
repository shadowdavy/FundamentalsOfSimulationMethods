...
class TreeClass:
    def __init__(self,particles):
        ...
        #
        #################################################
        # appended code ... storage for accurate forces #
        #################################################
        self.accurateforces = np.zeros((len(particles), 3), dtype = float)
        self.nodecount = np.zeros(len(particles), dtype = int)
        # 
            ...
            for iz in range(0,2):
                for iy in range(0,2):
                    for ix in range(0,2):
                        ...
                        mass = mass + m
                        # center of mass
                        cenm = [sum(x) for x in zip(cenm, cm)]
            # for the correct center of mass we need to divide by the total mass
            cenm = [x / mass for x in cenm]
    ...
    def gforce(self,nodeid,pid,anglemax):
        ...
        if angle < anglemax or self.nodelist[nodeid].leaf == 1:
            ...
            # appanded code #
            #################
            # particle mass is assumed to be 1, m is total mass of the node
            # force approximation
            nodecm = np.array(cm)
            xarr = np.array(x)
            if (r != 0 and m != 0):
                force = (m / r**3) * (x - nodecm)  # need to add smothing factor of the potential on sheet
                self.nodecount[pid] += 1 
        else:
            #
    ...
    # compute all forces
    def forcesSum(self):
        ilen = len(self.particles)
        jlen = len(self.particles)
        for i in range(ilen):
            force = np.zeros(3)
            for j in range(jlen):
                if (i != j):
                    p1 = np.asarray(self.particles[i][0:3])
                    p2 = np.asarray(self.particles[j][0:3])
                    r = np.sqrt(np.dot(p1, p2))
                    force = force + (p1 - p2) / r**3
                    self.accurateforces[i][:] = force
